...
- **8-way facing**: the body TURNS with the aim (mouse) direction through
  eight facings from FIVE baked views — front, back, a true side profile,
  and two three-quarter turns (front-right, back-right); the three
  east-facing strips mirror for the western turns. Every layer (hair,
  helmets, armor) is drawn for all five views, so a full helm's eye slit
  wraps the front edge on a three-quarter turn and long hair falls down the
  back when you turn away. Walking along a world axis (the screen
  diagonals) shows a three-quarter turn — the facing players see most.
  Animation stays deliberately minimal: the walk cycle is the only body
  animation, and held weapons hover/point per the aim like always.
  Dev aid: `ARPG_DEVUI=face:N` (S/E/W/SE/SW/NE/NW) pins the facing for
  screenshot runs.
...
`ARPG_DEVUI=debug[,skills][,inventory][,drops][,loot][,elite[:rare|magic|affix+affix]][,shop][,shopgrid][,tree][,summons][,knight][,warp][,tutorial][,mace][,flamefx][,scroll[:rare|magic|warp_id+warp_id]][,portal[:same]][,tp:x;y]` (set `ARPG_STORY=1` to boot the story world instead of the test grounds)
opens panels at startup (`warp` jumps to the next campaign map and `tutorial` into
the Old Road three seconds in — debug commands `warp_tutorial`, `warp_home`; `scroll` hands out a sealed warp scroll and `portal` seals one straight onto the ruins' podium (debug `give_warp`, `open_portal`); `tp` teleports five seconds in; `drops` scatters one of every scroll shortly after
...
- `dotnet run -- --nettest` — the automated two-client sync test described above
  (818 checks, exit code 0 on success; `--nettest story` runs only the story sections — the road, the ruins, the portal loop — in well under a minute). It exercises `127.0.0.1`; LAN/ZeroTier use the
